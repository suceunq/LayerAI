...
const PLATE_COLOR: Record<"dark" | "light", string> = { dark: "#1c1c20", light: "#e1e2e6" };
const GRID_COLOR: Record<"dark" | "light", string> = { dark: "#3a3a42", light: "#c4c6cc" };
const GRID_COLOR_CENTER = "#2f80ed";
 ...
 */
export function buildBedPlate(printer: PrinterProfile, theme: "dark" | "light" = "dark"): THREE.Group {
  ...
  const plateMaterial = new THREE.MeshStandardMaterial({ color: PLATE_COLOR[theme], roughness: 0.9, metalness: 0.05 });
  ...
  const grid = new THREE.GridHelper(Math.max(bedWidth, bedDepth), divisions, GRID_COLOR_CENTER, GRID_COLOR[theme]);
